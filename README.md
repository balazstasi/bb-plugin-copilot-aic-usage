...
Displays the **current Copilot ACP session’s cumulative AI credits (AIC / AIU)** in BB’s thread header. Open the badge to see AIC used today on this host, premium requests used this month as a share of the allowance, and the monthly reset date. Expand Details for checkpoint times and data notes. Copilot does not publish remaining AIC. Updates arrive without refreshing the page. Copilot threads using either `acp-gh-copilot` or `acp-copilot` display the control.
...
- BB 0.43.4 or newer with Plugin SDK **0.5.9** APIs (`>=0.5.9 <0.6`). Experimental APIs may change; the exact development SDK is pinned.
...
Install BB's **GitHub Copilot** provider plugin (`gh-copilot`) from the plugin store, then check that `bb provider list` includes `acp-gh-copilot`. Select **GitHub Copilot** when starting a BB thread. The provider plugin manages its own ACP entry and launches `copilot --acp`.

Alternatively, configure Copilot manually as a custom ACP agent. In BB's **ACP providers** plugin settings, add this object to the **Custom agents** JSON array, preserving any other agents:
...
Use the absolute output of `command -v copilot` for `command` if the BB host daemon cannot find it on PATH. The manually configured ID `copilot` gives provider ID `acp-copilot`; the `gh-copilot` plugin gives `acp-gh-copilot`. This AIC plugin supports both.
...
The manual setting takes effect immediately. Keep Copilot and the host daemon under the same operating-system account. For remote environments, install and authenticate Copilot on that remote host too. Run `bb guide providers` for BB's current custom-agent configuration reference.

### Install this checkout

The published `v0.1.0` release supports only manually configured `acp-copilot` threads. Support for the marketplace provider `acp-gh-copilot` is in this checkout and needs a new release. Until then, install this checkout locally:
...
The supported `experimental_threadHeaderAction` slot places the badge in the **thread header action row**. SDK 0.5.9 has no public slot directly beside the existing context-window circle. This plugin uses no DOM injection, global selectors, core patches, or context-token substitution. Exact placement beside the circle requires a BB UI slot addition.
...
The header shows only this session’s AIC. The popover adds **AIC used today on this host** (local midnight, every Copilot session directory still on disk whose event file was touched today) and monthly **premium requests used / allowance (used percentage)**. Today is a local sum of checkpoint deltas, not a GitHub invoice and not remaining AIC. A session that started before midnight contributes only the increase since the last pre-midnight checkpoint; if a scan is incomplete or unreadable, today is unavailable rather than a partial total. Session folders without an event file are skipped until telemetry appears. The shared host total refreshes at most 15 seconds after a read, and watched session changes invalidate it immediately. The monthly count comes from Copilot’s `premium_interactions` snapshot. Chat and completions snapshots exist in telemetry but are typically unlimited and are not shown. Missing fields read “Unavailable.” The latest saved checkpoint stays available when the session is idle; a disconnected BB realtime connection marks the displayed snapshot stale. Expand Details for checkpoint and monthly-snapshot timestamps and any daily-scan failure reason.

## Architecture and exact correlation

1. `server.ts` resolves the requested BB thread using the public SDK, checks for `acp-copilot` or `acp-gh-copilot`, and obtains `environment.hostId`.
...
5. Each viewed session directory gets a daemon-native filesystem watch. A bounded incremental reader extracts only checkpoint/quota fields for **this session**. A separate bounded reverse scan of sibling UUID directories whose `events.jsonl` was touched today sums AIC used since local midnight. That today total is a distinct field; it never replaces the current session’s AIC. Workspace files and process environments are unnecessary and are not read.
...
- Latest identity means the most recently recorded Copilot session. A stopped or idle session still shows its latest saved cumulative total. BB provider changes are rechecked on every request.
...
- A public provider identity API is **not** a blocker on SDK 0.5.9: the filtered identity event API supplies the exact identifier. A dedicated stable identity endpoint would reduce experimental API dependence. Direct circle adjacency requires a new frontend slot.
